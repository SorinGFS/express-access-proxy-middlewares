...
const handleError = (err, req, res, next) => {
    ...
    if (req.server && req.server.errorLogs) {
        ...
        if (Array.isArray(req.server.errorLogs)) {
            ...
            req.accessDb.controller('errors').insertOne({ time: new Date(), ...log });
        } else {
            req.accessDb.controller('errors').insertOne({ time: new Date(), ip: req.ip, ips: req.ips, method: req.method, protocol: req.protocol, hostname: req.hostname, url: req.url, message: message });
        }
    }
